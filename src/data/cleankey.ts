...
// Used for <meta description> and OG.
export const META_DESCRIPTION =
	'CleanKey is a macOS menu-bar app: a free keyboard and trackpad lock for cleaning, plus a Pro keep-awake with an AI coding mode that stays awake while your agent works and then notifies, runs a Shortcut, locks, and sleeps when it is done.';
...
export const FREE_VS_PRO: FeatureRow[] = [
	...
	{ feature: 'Lock scope: keyboard+trackpad or keyboard-only', free: true, pro: true },
	...
	{ feature: 'Battery guards (only-while-charging, % cutoff)', free: false, pro: true },
	...
	{ feature: 'End-of-session automations (notify, run Shortcut, lock, sleep)', free: false, pro: true },
];

// The flagship feature gets a dedicated block with the hook snippet; the rest are short cards.
export const AI_CODING_MODE = {
	title: 'AI coding mode (the flagship)',
	body: [
...
export const AUTOMATIONS = {
	...
	actions: [
		{ name: 'Notify', body: 'Post a notification so you know the session ended, even from another room.' },
		{ name: 'Run Shortcut', body: 'Run any macOS Shortcut by name. Commit, push, post to Slack, anything Shortcuts can do, with no extra permissions and no change to how CleanKey is signed.' },
		{ name: 'Lock', body: 'Lock the keyboard and trackpad with CleanKey’s own lock, so nothing disturbs the Mac after the run.' },
		{ name: 'Sleep', body: 'Put the Mac to sleep once the rest of the sequence has finished.' },
	] as AutomationAction[],
	safety:
...
export const COMPARISON_COLS = ['CleanKey', 'Amphetamine', 'KeepingYouAwake', 'Adrafinil (CLI)'];
export const COMPARISON_ROWS: ComparisonRow[] = [
	{ label: 'Keyboard + trackpad lock', values: ['Yes', 'No', 'No', 'No'] },
	{ label: 'Keep-awake', values: ['Yes', 'Yes', 'Yes', 'Yes'] },
	{ label: 'Battery % cutoff', values: ['Yes', 'Yes', 'Yes (Low Power Mode)', 'Not documented'] },
	{ label: 'Thermal pause', values: ['Yes', 'No (relies on macOS)', 'No', 'Yes'] },
	{ label: 'Clamshell (lid closed)', values: ['Yes (leased helper)', 'Yes (v5.0+)', 'No', 'Yes (XPC helper)'] },
	{ label: 'AI-agent-driven awake', values: ['Yes (file heartbeat)', 'No', 'No', 'Yes (process/agent)'] },
	{ label: 'Menu-bar GUI app', values: ['Yes', 'Yes', 'Yes', 'No (command line)'] },
	{ label: 'Notarized paid app', values: ['Yes', 'Free', 'Free / open source', 'Free / open source'] },
];
...
export const FAQ: FaqItem[] = [
	{ q: 'Does it need Accessibility permission?', a: 'Yes, the lock uses it to block input. If the permission is ever revoked, the lock releases itself.' },
	{ q: 'Will keep-awake drain my battery?', a: 'Only if you let it. Set a battery cutoff or “only while charging”, and it steps aside when unplugged or low.' },
	{ q: 'Does it work with the lid closed?', a: 'Yes, with the clamshell option. It uses a signed helper that clears the awake state automatically if the app stops.' },
	{ q: 'Does AI coding mode work with Codex, not just Claude Code?', a: 'Yes. It watches a file, so any tool whose hooks can touch that file works, including Codex.' },
	{ q: 'What can it do when my agent finishes?', a: 'End-of-session automations run a sequence you choose: notify, run a macOS Shortcut, lock, then sleep. A countdown you can cancel runs first, and nothing happens if you move the mouse or the agent starts working again.' },
	{ q: 'What if my agent crashes mid-task?', a: 'The heartbeat goes stale and the Mac sleeps after the idle timeout. Nothing is left stuck awake.' },
	{ q: 'Will it get hot running overnight?', a: 'Thermal protection pauses keep-awake when the Mac gets too warm and resumes when it cools.' },
	{ q: 'How do updates work?', a: 'Built-in, signed, one click. The app checks, downloads, verifies, installs, and relaunches.' },
	{ q: 'How do I buy CleanKey Pro?', a: 'Buy a license through Polar, then paste the key into the app’s License pane. Every Pro feature unlocks, and the lock stays free either way.' },
...
export const SHOT_LIST: Shot[] = [
	...
	{ key: 'hero-ai-coding-mode.gif', caption: 'Awake while it codes. Asleep when it’s done.' },
	{
...
export const POLAR_CHECKOUT_URL = 'https://buy.polar.sh/polar_cl_PBbomGVla4ni0vXmxjgo5myXmxtWNqyRCb2Og4K4BvS';
